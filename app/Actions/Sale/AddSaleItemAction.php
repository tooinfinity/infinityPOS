...
final readonly class AddSaleItemAction
{
    /**
     * @throws Throwable
     */
    ...
    private function validateStockAvailability(SaleItemData $data): void
    {
        ...
        if ($batch->quantity < $data->quantity) {
            throw new RuntimeException(
                "Insufficient stock in batch. Required: {$data->quantity}, Available: {$batch->quantity}"
            );
        }
    }

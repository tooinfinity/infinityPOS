...
type SaleReturnRow = App.Models.SaleReturn & {
    customer?: App.Models.Customer | null;
};

interface Props extends Inertia.SharedData {
    saleReturns: Paginated<SaleReturnRow>;
    filters: Filters;
}

export default function SaleReturnsIndex({ saleReturns, filters = {} }: Props) {
    const [completeReturn, setCompleteReturn] =
        useState<App.Models.SaleReturn | null>(null);
    ...
    const columns: ColumnDef<SaleReturnRow, unknown>[] = [
        {
            ...
            cell: ({ row }) => (
                <button
                    ...
                    onClick={() =>
                        router.visit(
                            SaleReturnController.show.url({
                                saleReturn: row.original.id,
                            }),
                        )
                    }
                >
                    ...
                    onClick={() =>
                        ...
                        router.visit(
                            SaleController.show.url({
                                sale: row.original.sale.id,
                            }),
                        )
                    }
                >
                    {row.original.sale?.reference_no ?? '—'}
                </button>
            ),
        },
        {
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="space-y-5">
                    ...
                    <DataTable
                        columns={columns}
                        data={saleReturns}
                        baseUrl={SaleReturnController.index.url()}
                        filters={filters}
                    />
                </div>
                ...
                {completeReturn && (
                    <ActionDialog
                        ...
                        actionRoute={CompleteSaleReturnController.url({
                            saleReturn: completeReturn.id,
                        })}
                        title="Complete this return?"
                        description={`Return ${completeReturn.reference_no} will be completed and stock will be restored.`}
